...
bool FORMULA(reference_scaled_double,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
  ( const int m_nFractalType, const int m_nPower
  ...
  )
{
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
  {
    ...
    mpfr_t Xr; mpfr_init2(Xr, bits); mpfr_set_d(Xr, g_SeedR, MPFR_RNDN);
    mpfr_t Xi; mpfr_init2(Xi, bits); mpfr_set_d(Xi, g_SeedI, MPFR_RNDN);
    double Xrd = mpfr_get(Xr, double(0.0), MPFR_RNDN);
    double Xid = mpfr_get(Xi, double(0.0), MPFR_RNDN);
    mpfr_t Xr2; mpfr_init2(Xr2, bits); mpfr_sqr(Xr2, Xr, MPFR_RNDN);
    mpfr_t Xi2; mpfr_init2(Xi2, bits); mpfr_sqr(Xi2, Xi, MPFR_RNDN);
    mpfr_t Xrn; mpfr_init2(Xrn, bits);
    mpfr_t Xin; mpfr_init2(Xin, bits);
...
#define LOOP \
      ...
      mpfr_sqr(Xi2, Xi, MPFR_RNDN); \
      m_nRDone++; \
      Xrd = mpfr_get(Xr, double(0.0), MPFR_RNDN); \
      Xid = mpfr_get(Xi, double(0.0), MPFR_RNDN); \
      old_absval = abs_val; \
      abs_val = double(g_real * Xrd * Xrd + g_imag * Xid * Xid); \
      const double Xz = abs_val * glitch; \
      m_db_dxr[i] = Xrd; \
...
<xsl:when test="derivative/@t='M'">
    long double dxa = daa, dxb = dab, dya = dba, dyb = dbb;
    long double dxan = 0, dxbn = 0, dyan = 0, dybn = 0;
</xsl:when>
</xsl:choose>
<xsl:choose>
<xsl:when test="reference/@t='C'">
...
@rd   {
        ...
        const complex&lt;double&gt; Xx(Xxr, Xxi), A(g_FactorAR, g_FactorAI);
<xsl:choose>
<xsl:when test="derivative/@t='C'">
        const complex&lt;long double&gt; d(dr, di), d0(daa, dba); <!-- FIXME matrix derivatives -->
...
<xsl:when test="derivative/@t='C'">
        ...
        const complex&lt;long double&gt; d0(daa, dba); <!-- FIXME matrix derivatives -->
        complex&lt;long double&gt; dn(0.0, 0.0);
        <xsl:value-of select="derivative" />
        drn = dn.m_r; din = dn.m_i;
</xsl:when>
...
bool FORMULA(reference_scaled_long_double,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
  ( const int m_nFractalType, const int m_nPower
  ...
  )
{
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
  {
    ...
    mpfr_t Xr; mpfr_init2(Xr, bits); mpfr_set_d(Xr, g_SeedR, MPFR_RNDN);
    mpfr_t Xi; mpfr_init2(Xi, bits); mpfr_set_d(Xi, g_SeedI, MPFR_RNDN);
    long double Xrd = mpfr_get(Xr, (long double)(0.0), MPFR_RNDN);
    long double Xid = mpfr_get(Xi, (long double)(0.0), MPFR_RNDN);
    mpfr_t Xr2; mpfr_init2(Xr2, bits); mpfr_sqr(Xr2, Xr, MPFR_RNDN);
    mpfr_t Xi2; mpfr_init2(Xi2, bits); mpfr_sqr(Xi2, Xi, MPFR_RNDN);
    mpfr_t Xrn; mpfr_init2(Xrn, bits);
    mpfr_t Xin; mpfr_init2(Xin, bits);
...
#define LOOP \
      ...
      mpfr_sqr(Xi2, Xi, MPFR_RNDN); \
      m_nRDone++; \
      Xrd = mpfr_get(Xr, (long double)(0.0), MPFR_RNDN); \
      Xid = mpfr_get(Xi, (long double)(0.0), MPFR_RNDN); \
      old_absval = abs_val; \
      abs_val = double(g_real * Xrd * Xrd + g_imag * Xid * Xid); \
      const double Xz = abs_val * glitch; \
      m_db_dxr[i] = Xrd; \
...
<xsl:when test="derivative/@t='M'">
    floatexp dxa = daa, dxb = dab, dya = dba, dyb = dbb;
    floatexp dxan = 0, dxbn = 0, dyan = 0, dybn = 0;
</xsl:when>
</xsl:choose>
<xsl:choose>
<xsl:when test="reference/@t='C'">
...
<xsl:when test="reference/@t='C'">
@rd   {
        const long double Xxr = Xrd; (void) Xxr;
        const long double Xxi = Xid; (void) Xxi;
        const complex&lt;long double&gt; Xx(Xxr, Xxi), A(g_FactorAR, g_FactorAI);
<xsl:choose>
<xsl:when test="derivative/@t='C'">
        const complex&lt;floatexp&gt; d(dr, di), d0(daa, dba); <!-- FIXME matrix derivatives -->
...
<xsl:when test="derivative/@t='C'">
        ...
        const complex&lt;floatexp&gt; d0(daa, dba); <!-- FIXME matrix derivatives -->
        complex&lt;floatexp&gt; dn(0.0, 0.0);
        <xsl:value-of select="derivative" />
        drn = dn.m_r; din = dn.m_i;
</xsl:when>
...
<xsl:when test="derivative/@t='M'">
    dr0 = (floatexp(Xrd) * dxa + floatexp(Xid) * dya) / sqrt(test1);
    di0 = (floatexp(Xrd) * dxb + floatexp(Xid) * dyb) / sqrt(test1);
</xsl:when>
</xsl:choose>
    return true;
  }
...
<xsl:for-each select="//scaled/..">
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
    return FORMULA(reference_scaled_double,<xsl:value-of select="../@type" />, <xsl:value-of select="@power" />)
      ( m_nFractalType, m_nPower, m_db_dxr, m_db_dxi, m_db_z, m_bStop, m_nRDone, m_nGlitchIter, m_nMaxIter
      , Cr0, Ci0, g_SeedR, g_SeedI, g_FactorAR, g_FactorAI, terminate, g_real, g_imag, m_bGlitchLowTolerance
      , antal, test1, test2
      );
</xsl:for-each>
  return false;
...
<xsl:for-each select="//scaled/..">
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
    return FORMULA(reference_scaled_double,<xsl:value-of select="../@type" />, <xsl:value-of select="@power" />)
      ( m_nFractalType, m_nPower, m_db_dxr, m_db_dxi, m_db_z, m_bStop, m_nRDone, m_nGlitchIter, m_nMaxIter
      , Cr0, Ci0, g_SeedR, g_SeedI, g_FactorAR, g_FactorAI, terminate, g_real, g_imag, m_bGlitchLowTolerance
      , antal, test1, test2, dr0, di0, daa, dab, dba, dbb
      );
</xsl:for-each>
  return false;
}

bool reference_scaled_long_double
  ( const int m_nFractalType, const int m_nPower
  , long double *m_db_dxr, long double *m_db_dxi, double *m_db_z
  , int &amp;m_bStop, int &amp;m_nRDone, int &amp;m_nGlitchIter, int &amp;m_nMaxIter
  , const CFixedFloat &amp;Cr0, const CFixedFloat &amp;Ci0
...
<xsl:for-each select="//scaled/..">
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
    return FORMULA(reference_scaled_long_double,<xsl:value-of select="../@type" />, <xsl:value-of select="@power" />)
      ( m_nFractalType, m_nPower, m_db_dxr, m_db_dxi, m_db_z, m_bStop, m_nRDone, m_nGlitchIter, m_nMaxIter
      , Cr0, Ci0, g_SeedR, g_SeedI, g_FactorAR, g_FactorAI, terminate, g_real, g_imag, m_bGlitchLowTolerance
      , antal, test1, test2
      );
  ...
  )
{
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
  {
    ...
    const double Ai = g_FactorAI;
    const complex&lt;double&gt; A = { Ar, Ai };
    const complex&lt;double&gt; c = { cr0, ci0 };
    (void) Ar; // -Wunused-variable
    (void) Ai; // -Wunused-variable
    (void) A; // -Wunused-variable
    (void) c; // -Wunused-variable
...
<xsl:when test="scaled/@t='R'">
      double cr = cr0, ci = ci0, xr = xr0, xi = xi0;
@d    {
...
bool FORMULA(perturbation2_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int2 &amp;antal0, double2 &amp;test10, double2 &amp;test20, int2 &amp;bGlitch0
  , double m_nBailout2, const int nMaxIter
    ...
    if (all2(antal == antal[0]))
    {
      for (; antal[0] + chunksize - 1 &lt; nMaxIter; antal = antal + chunksize)
      {
        ...
        for (int q = 0; q &lt; chunksize; ++q)
        {
          ...
          const double Xi = m_db_dxi[antal[0] + q];
          const double Xz = m_db_z[antal[0] + q];
          double2 Xxr = Xr + xr0 * s;
          double2 Xxi = Xi + xi0 * s;
          test2 = test1;
          if (no_g)
          {
            test1 = Xxr * Xxr + Xxi * Xxi;
...
bool FORMULA(perturbation4_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int4 &amp;antal0, double4 &amp;test10, double4 &amp;test20, int4 &amp;bGlitch0
  , double m_nBailout2, const int nMaxIter
    ...
    if (all4(antal == antal[0]))
    {
      for (; antal[0] + chunksize - 1 &lt; nMaxIter; antal = antal + chunksize)
      {
        ...
        for (int q = 0; q &lt; chunksize; ++q)
        {
          ...
          const double Xi = m_db_dxi[antal[0] + q];
          const double Xz = m_db_z[antal[0] + q];
          double4 Xxr = Xr + xr0 * s;
          double4 Xxi = Xi + xi0 * s;
          test2 = test1;
          if (no_g)
          {
            test1 = Xxr * Xxr + Xxi * Xxi;
...
bool FORMULA(perturbation8_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int8 &amp;antal0, double8 &amp;test10, double8 &amp;test20, int8 &amp;bGlitch0
  , double m_nBailout2, const int nMaxIter
    ...
    if (all8(antal == antal[0]))
    {
      for (; antal[0] + chunksize - 1 &lt; nMaxIter; antal = antal + chunksize)
      {
        ...
        for (int q = 0; q &lt; chunksize; ++q)
        {
          ...
          const double Xi = m_db_dxi[antal[0] + q];
          const double Xz = m_db_z[antal[0] + q];
          double8 Xxr = Xr + xr0 * s;
          double8 Xxi = Xi + xi0 * s;
          test2 = test1;
          if (no_g)
          {
            test1 = Xxr * Xxr + Xxi * Xxi;
...
bool FORMULA(perturbation16_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int16 &amp;antal0, double16 &amp;test10, double16 &amp;test20, int16 &amp;bGlitch0
  , double m_nBailout2, const int nMaxIter
    ...
    if (all16(antal == antal[0]))
    {
      for (; antal[0] + chunksize - 1 &lt; nMaxIter; antal = antal + chunksize)
      {
        ...
        for (int q = 0; q &lt; chunksize; ++q)
        {
          ...
          const double Xi = m_db_dxi[antal[0] + q];
          const double Xz = m_db_z[antal[0] + q];
          double16 Xxr = Xr + xr0 * s;
          double16 Xxi = Xi + xi0 * s;
          test2 = test1;
          if (no_g)
          {
            test1 = Xxr * Xxr + Xxi * Xxi;
...
bool perturbation2_scaled_double
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int2 &amp;antal0, double2 &amp;test10, double2 &amp;test20, int2 &amp;bGlitch0
  , double m_nBailout2, const int nMaxIter
...
<xsl:for-each select="//scaled/..">
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
    return FORMULA(perturbation2_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
      ( m_nFractalType, m_nPower
      , m_db_dxr, m_db_dxi, m_db_z
      , antal0, test10, test20, bGlitch0
      , m_nBailout2, nMaxIter
...
bool perturbation4_scaled_double
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int4 &amp;antal0, double4 &amp;test10, double4 &amp;test40, int4 &amp;bGlitch0
  , double m_nBailout4, const int nMaxIter
  , const int m_bNoGlitchDetection, const double g_real, const double g_imag
  , const double g_FactorAR, const double g_FactorAI
  , double4 &amp;xr00, double4 &amp;xi00
  , const double4 &amp;cr0, const double4 &amp;ci0
...
<xsl:for-each select="//scaled/..">
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
    return FORMULA(perturbation4_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
      ( m_nFractalType, m_nPower
      , m_db_dxr, m_db_dxi, m_db_z
      , antal0, test10, test40, bGlitch0
      , m_nBailout4, nMaxIter
      , m_bNoGlitchDetection, g_real, g_imag
      , g_FactorAR, g_FactorAI
      , xr00, xi00
      , cr0, ci0
...
bool perturbation8_scaled_double
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int8 &amp;antal0, double8 &amp;test10, double8 &amp;test80, int8 &amp;bGlitch0
  , double m_nBailout8, const int nMaxIter
  , const int m_bNoGlitchDetection, const double g_real, const double g_imag
  , const double g_FactorAR, const double g_FactorAI
  , double8 &amp;xr00, double8 &amp;xi00
  , const double8 &amp;cr0, const double8 &amp;ci0
...
<xsl:for-each select="//scaled/..">
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
    return FORMULA(perturbation8_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
      ( m_nFractalType, m_nPower
      , m_db_dxr, m_db_dxi, m_db_z
      , antal0, test10, test80, bGlitch0
      , m_nBailout8, nMaxIter
      , m_bNoGlitchDetection, g_real, g_imag
      , g_FactorAR, g_FactorAI
      , xr00, xi00
      , cr0, ci0
...
bool perturbation16_scaled_double
  ( int m_nFractalType, int m_nPower
  , const double *m_db_dxr, const double *m_db_dxi, const double *m_db_z
  , int16 &amp;antal0, double16 &amp;test10, double16 &amp;test160, int16 &amp;bGlitch0
  , double m_nBailout16, const int nMaxIter
  , const int m_bNoGlitchDetection, const double g_real, const double g_imag
  , const double g_FactorAR, const double g_FactorAI
  , double16 &amp;xr00, double16 &amp;xi00
  , const double16 &amp;cr0, const double16 &amp;ci0
...
<xsl:for-each select="//scaled/..">
  if (m_nFractalType == <xsl:value-of select="../@type" /> &amp;&amp; m_nPower == <xsl:value-of select="@power" />)
    return FORMULA(perturbation16_scaled,<xsl:value-of select="../@type" />,<xsl:value-of select="@power" />)
      ( m_nFractalType, m_nPower
      , m_db_dxr, m_db_dxi, m_db_z
      , antal0, test10, test160, bGlitch0
      , m_nBailout16, nMaxIter
      , m_bNoGlitchDetection, g_real, g_imag
      , g_FactorAR, g_FactorAI
      , xr00, xi00
      , cr0, ci0
