...
then
  ...
  then
    ...
    ./configure --host=x86_64-w64-mingw32 --prefix=$HOME/win64 --with-gmp-build=../gmp-6.2.1 --enable-static --disable-shared
    make -j $NCPUS
    make install
    make check
  fi
...
then
  ...
  then
    ...
    cd mpfr-*/
    #patch -N -Z -p1 < ../allpatches
    ./configure --host=i686-w64-mingw32 --prefix=$HOME/win32 --with-gmp-build=../gmp-6.2.1 --enable-static --disable-shared
    make -j $NCPUS
    make install
    make check
  fi
  ...
  echo "  $0 64      # build all 64bit libs"
  echo "  $0 32      # build all 32bit libs"
  echo "  $0 64 lib  # build 64bit lib"
  echo "  $0 32 lib  # build 32bit lib"
  exit 1
fi
